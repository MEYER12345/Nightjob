     ...
     d*intprod         ds                  qualified inz occurs(100)
     d intprod         ds                  qualified inz dim(100)
     d   ip_comp                      1
     d   ip_locn                      5
     d   ip_birth                      d
     d   ip_lastupd                    d
     ...
     d  yearend_52                    1
      ...
      /free
       ...
       // SqlEr3 holds number of records SQL fetched
           for Ndx = 1 to RowsFetched;
              ...
              //%occur(intprod) = Ndx;
              @ip_comp= intprod(ndx).ip_comp;
              @ip_locn = intprod(ndx).ip_locn;
              @ip_supl = intprod(ndx).ip_supl;
              @ip_suplsub = intprod(ndx).ip_suplsub;
              @ip_prod = intprod(ndx).ip_prod;
       ...
       //  change in location
              if intprod(ndx).ip_locn <> locn;
                 locn    = intprod(ndx).ip_locn;
       ...
       // change in supplier ID
              ...
              #iplocn = intprod(ndx).ip_locn;
              #ipsupl = intprod(ndx).ip_supl;
              #ipsuplsub = intprod(ndx).ip_suplsub;
       ...
       //   look for product record at this location
                          px_comp    = intprod(ndx).ip_comp;
                          px_locn    = intprod(ndx).ip_locn;
                          px_prod    = intprod(ndx).ip_prod;
       ...
       *inlr = *on;
       ...
       pr_suplorg = intprod(ndx).ip_supl;
       pr_suplors = intprod(ndx).ip_suplsub;

       pr_suplusr = intprod(ndx).ip_suplusr;
       pr_suplusb = intprod(ndx).ip_suplusb;
       ...
       //  manufactor out flag
       pr_mfgout  = intprod(ndx).ip_mfgout;

       //  product sequence
       pr_prodseq = intprod(ndx).ip_prodseq;

       //  product description information

       //      product must have first description
       if intprod(ndx).ip_desc1 = *blanks;
          lg_logtype = 'K';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
          ...
          sp_name    = saved_name;
          sp_buyr    = saved_buyr;
          intprod(ndx).ip_desc1   = '** Missing description **';
          exsr insert_logprod;
       endif;
       pr_desc1   = intprod(ndx).ip_desc1;

       //     only update description 2 if data passed from host system
       if intprod(ndx).ip_desc2 <> *blanks;
          pr_desc2 = intprod(ndx).ip_desc2;
       endif;

       //     only update manufactor if data passed from host system
       if intprod(ndx).ip_mfg <> *blanks;
          pr_mfg = intprod(ndx).ip_mfg;
       endif;

       //      default catalog with ndc/upc code
       pr_catalog = intprod(ndx).ip_ndc_upc;

       //      only update ndc/upc code if data passed from host system
       if intprod(ndx).ip_ndc_upc <> *blanks;
          pr_ndc_upc = intprod(ndx).ip_ndc_upc;
       endif;

      //      only update unit of measure if data passed from host system
       if intprod(ndx).ip_uom <> *blanks;
          pr_uom = intprod(ndx).ip_uom;
       endif;

       //      only update pack size if data passed from host system
       if intprod(ndx).ip_packsiz <> *blanks;
          pr_packsiz = intprod(ndx).ip_packsiz;
       endif;

       //      only update ti/hi if data passed from host system
       if intprod(ndx).ip_tihi <> *blanks;
          pr_tihi = intprod(ndx).ip_tihi;
       endif;

       //      only update warehouse location if data passed from host system
       if intprod(ndx).ip_whslocn <> *blanks;
          pr_whslocn = intprod(ndx).ip_whslocn;
       endif;

       //  product status
       //      product status passed must be an 'R' or 'D'
       //        if not, then an 'R' is assumed
       if intprod(ndx).ip_status <> 'R' AND intprod(ndx).ip_status <> 'D';
          lg_logtype = 'V';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       //      either 'R', 'L', or 'S'
       if intprod(ndx).ip_status  = 'R' AND pr_sysstat = 'D';
      ...
      /copy k3s_c160
       ...
       //  group 1 logic
       pr_group1  = intprod(ndx).ip_group1;
       ...
       //  group 2 logic
       pr_group2  = intprod(ndx).ip_group2;
       ...
       //  group 3 logic
       pr_group3  = intprod(ndx).ip_group3;
       ...
       //  group 4 logic
       pr_group4  = intprod(ndx).ip_group4;
       ...
       //  group 5 logic
       pr_group5  = intprod(ndx).ip_group5;
       ...
       // evalr ip_desc1 = %editc(ip_costdiv:'X');
          exsr insert_logprod;
          intprod(ndx).ip_costdiv = 1;
       endif;

       //      don't allow purchase price to be negative
       if intprod(ndx).ip_costreg < 0;
          lg_logtype = 'I';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       if intprod(ndx).ip_costreg = 0 and cost_0 = 1;
          lg_logtype = 'c';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
          ...
          sp_buyr    = saved_buyr;
          if cost_fill  = 1;
             intprod(ndx).ip_costreg = cost_value;
             intprod(ndx).ip_sales   = cost_value;
          endif;
          exsr insert_logprod;
       endif;

       //      don't allow sales price to be negative
       if intprod(ndx).ip_sales   < 0;
          lg_logtype = 'J';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       // move      ip_sales      ip_desc1
          exsr insert_logprod;
          intprod(ndx).ip_sales   = 0;
       endif;

       //      don't allow sales price to be < cost
       if intprod(ndx).ip_sales <
              intprod(ndx).ip_costreg / intprod(ndx).ip_costdiv;
          lg_logtype = 'Y';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       if pr_ovrcreg > 0 and
          ...
          intprod(ndx).ip_costreg = pr_ovrcreg;
          intprod(ndx).ip_costdiv = pr_ovrcdiv;
       endif;

       if pr_costreg <> intprod(ndx).ip_costreg;
       ...
       //    update costs
       pr_costreg = intprod(ndx).ip_costreg;
       pr_costdiv = intprod(ndx).ip_costdiv;
       pr_costeac = intprod(ndx).ip_costreg / intprod(ndx).ip_costdiv;

       //    update sales price
       pr_sales   = intprod(ndx).ip_sales;

       //    quantity logic

       //    on hand section
       if intprod(ndx).ip_qtyohnd < 0;
          lg_logtype = 'F';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       if delay_days = 1;
          if pr_sysstat <> 'D' AND
             ...
             if pr_qtyohnd > 0 AND
                intprod(ndx).ip_qtyohnd = 0 AND
                pr_birth   < delay_test AND
                intprod(ndx).ip_dlysale > 0;

                clear delayuntil;
                callp K3S_3860(intprod(ndx).ip_comp:
                               intprod(ndx).ip_locn:
                               intprod(ndx).ip_supl:
                               intprod(ndx).ip_suplsub:
                               sp_buyr:
                               pr_ansaleu:
                               lc_sysdate:
                               delayuntil);
       ...
       pr_qtyohnd = intprod(ndx).ip_qtyohnd;

       //     on order section
       if intprod(ndx).ip_qtyoord < 0;
          lg_logtype = 'G';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       // move      ip_qtyoord    ip_desc1
          exsr insert_logprod;
          clear intprod(ndx).ip_qtyoord;
       endif;
       pr_qtyoord = intprod(ndx).ip_qtyoord;

       //     back order section
       if intprod(ndx).ip_qtyback < 0;
          lg_logtype = 'H';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
        ...
        //move      ip_qtyback    ip_desc1
          exsr insert_logprod;
          clear intprod(ndx).ip_qtyback;
       endif;
       pr_qtyback = intprod(ndx).ip_qtyback;

       //  buying multiple
       //      is buying multiple being passed less than 1?
       if intprod(ndx).ip_buymult < 1;
          lg_logtype = 'M';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       // ip_desc1   = 'Invalid buying multiple passed';
          exsr insert_logprod;
          intprod(ndx).ip_buymult = 1;
       endif;
       pr_buymuli = intprod(ndx).ip_buymult;
       pr_buymult = intprod(ndx).ip_buymult;
       if pr_buymulo <> *zeros;
       ...
       // ip_desc1   = 'Invalid min. quantity passed';
          exsr insert_logprod;
          intprod(ndx).ip_minqty  = 1;
       endif;
       pr_minqty  = intprod(ndx).ip_minqty;
       if pr_minqtyo <> *zeros;
       ...
       // ip_desc1   = 'Negative purchase increment';
          exsr insert_logprod;
          intprod(ndx).ip_purincr = 0;
       endif;
       pr_purincr = intprod(ndx).ip_purincr;
       if pr_purinco <> *zeros;
       ...
       if intprod(ndx).ip_disothr < 0;
          ...
          sp_buyr    = saved_buyr;
          exsr insert_logprod;
          intprod(ndx).ip_disothr = 0;
       endif;
       pr_disothr = intprod(ndx).ip_disothr;
       if pr_disotho <> *zeros;
       ...
       if intprod(ndx).ip_disunt7 < 0;
          ...
          sp_buyr    = saved_buyr;
          exsr insert_logprod;
          intprod(ndx).ip_disunt7 = 0;
       endif;
       pr_disunt7 = intprod(ndx).ip_disunt7;
       if pr_disun7o <> *zeros;
       ...
       if intprod(ndx).ip_disunt8 < 0;
          ...
          sp_buyr    = saved_buyr;
          exsr insert_logprod;
          intprod(ndx).ip_disunt8 = 0;
       endif;
       pr_disunt8 = intprod(ndx).ip_disunt8;
       if pr_disun8o <> *zeros;
       ...
       if intprod(ndx).ip_disunt9 < 0;
          ...
          sp_buyr    = saved_buyr;
          exsr insert_logprod;
          intprod(ndx).ip_disunt9 = 0;
       endif;
       pr_disunt9 = intprod(ndx).ip_disunt9;
       if pr_disun9o <> *zeros;
       ...
       // ip_desc1   = 'Negative rebate passed';
          exsr insert_logprod;
          intprod(ndx).ip_rebate  = 0;
       endif;
       pr_rebate  = intprod(ndx).ip_rebate;

       //  convenience pack
       //      is convenience pack being passed less than 0?
       if intprod(ndx).ip_convpak  < 0;
          lg_logtype = 'Q';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       // ip_desc1   = 'Neg. convenience pack passed';
          exsr insert_logprod;
          intprod(ndx).ip_convpak = 0;
       endif;
       pr_convpak = intprod(ndx).ip_convpak;
       if pr_convpko <> *zeros;
       ...
       // ip_desc1   = 'Weight divisor less than 1';
          exsr insert_logprod;
          intprod(ndx).ip_weightd = 1;
       endif;

       //      is weight being passed less than 0?
       if intprod(ndx).ip_weight  < 0;
          lg_logtype = 'R';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       // ip_desc1   = 'Negative weight being passed';
          exsr insert_logprod;
          intprod(ndx).ip_weight  = 0;
       endif;

       pr_weight  = intprod(ndx).ip_weight;
       pr_weightd = intprod(ndx).ip_weightd;
       ...
       // ip_desc1   = 'Volume divisor less than 1';
          exsr insert_logprod;
          intprod(ndx).ip_volumed = 1;
       endif;

       //    is volume being passed less than 0?
       if intprod(ndx).ip_volume  < 0;
          lg_logtype = 'T';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       // ip_desc1   = 'Negative volume being passed';
          exsr insert_logprod;
          intprod(ndx).ip_volume  = 0;
       endif;

       pr_volume  = intprod(ndx).ip_volume;
       pr_volumed = intprod(ndx).ip_volumed;
       ...
       // move      ip_dlysale    ip_desc1
          exsr insert_logprod;
          clear intprod(ndx).ip_dlysale;
       endif;

       //      is data valid (negative outs being passed?)
       if intprod(ndx).ip_dlyouts < 0;
          lg_logtype = 'E';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       //      add daily sales
       pr_accsale += intprod(ndx).ip_dlysale;
       //      add daily lost sales
       pr_accouts += intprod(ndx).ip_dlyouts;
       //      demand for period
       ...
       //  days out logic
                if (intprod(ndx).ip_qtyohnd <= pr_daysunt) and
                   (pr_sysstat <> 'N') and
                   (pr_forcast > *zeros) and
                   continue = 1;
       ...
       //  contract existance
       pr_contflg = intprod(ndx).ip_contflg;

       //  car count flag (must be 0=no or 1=yes)
       if car_update = 1;
          pr_carcoun = intprod(ndx).ip_carcoun;
          if pr_carcoun <> 0 and
             pr_carcoun <> 1;
             pr_carcoun =  0;
          endif;
       ...
       //    ip_desc1   = 'Invalid flag alt srce process';
             intprod(ndx).ip_procalt = 0;
             exsr insert_logprod;
          endif;
          pr_procalt = intprod(ndx).ip_procalt;
       endif;
       ...
       //     go check for another deal
          ...
          callp K3S_5120(intprod(ndx).ip_comp:
                         intprod(ndx).ip_locn:
                         intprod(ndx).ip_supl:
                         intprod(ndx).ip_suplsub:
                         intprod(ndx).ip_prod:
       ...
       //     monthly and 13-four weekly products
          callp K3S_3510(intprod(ndx).ip_comp:
                         intprod(ndx).ip_locn:
                         px_supl:
                         px_suplsub:
                         intprod(ndx).ip_prod:
                         ...
                         intprod(ndx).ip_supl:
                         intprod(ndx).ip_suplsub:
                         intprod(ndx).ip_suplusr:
                         intprod(ndx).ip_suplusb);

       else;

       //     weekly products
          callp K3S_3511(intprod(ndx).ip_comp:
                         intprod(ndx).ip_locn:
                         px_supl:
                         px_suplsub:
                         intprod(ndx).ip_prod:
       ...
       pr_supl    = intprod(ndx).ip_supl;
       pr_suplsub = intprod(ndx).ip_suplsub;
       pr_suplusr = intprod(ndx).ip_suplusr;
       pr_suplusb = intprod(ndx).ip_suplusb;
       pr_suplorg = intprod(ndx).ip_supl;
       pr_suplors = intprod(ndx).ip_suplsub;
       ...
       // key information
       pr_comp    = intprod(ndx).ip_comp;
       pr_locn    = intprod(ndx).ip_locn;
       pr_supl    = intprod(ndx).ip_supl;
       pr_suplsub = intprod(ndx).ip_suplsub;
       pr_suplusr = intprod(ndx).ip_suplusr;
       pr_suplusb = intprod(ndx).ip_suplusb;
       ...
       //         should be:  1 = weekly
       //                     2 = monthly
       if intprod(ndx).ip_forctyp < 1 OR intprod(ndx).ip_forctyp > 3;
          lg_logtype = 'W';
          lg_prvsupl = *blanks;
          lg_prvsub  = *blanks;
          lg_prvsplu = *blanks;
       ...
       //       weekly forecasting
          when intprod(ndx).ip_forctyp = 1;
               pr_forcint = 52;
               pr_forcper = lc_perd52;
               pr_forcyr  = lc_year52;

       //       monthly forecasting
          when intprod(ndx).ip_forctyp = 2;
               pr_forcint = 12;
               pr_forcper = lc_perd12;
               pr_forcyr  = lc_year12;

       //       thirteen four-weekly forecasting
          when intprod(ndx).ip_forctyp = 3;
               pr_forcint = 13;
      ...
      /copy k3s_c160
       ...
       lg_comp    = intprod(ndx).ip_comp;
       lg_locn    = intprod(ndx).ip_locn;
       lg_buyr    = sp_buyr;
       lg_supl    = intprod(ndx).ip_supl;
       lg_suplsub = intprod(ndx).ip_suplsub;
       lg_suplusr = intprod(ndx).ip_suplusr;
       lg_suplusb = intprod(ndx).ip_suplusb;
       lg_suplnam = sp_name;
       lg_prod    = intprod(ndx).ip_prod;
       lg_desc1   = intprod(ndx).ip_desc1;
       ...
       if get_log_5 = '1' AND lg_logtype = '5';
          callp K3S_9010X1(intprod(ndx).ip_comp:
                           intprod(ndx).ip_locn:
                           intprod(ndx).ip_suplusr:
                           intprod(ndx).ip_suplusb:
                           intprod(ndx).ip_prod:

     ...
     f*_intprod if   e             disk
      * products

     f*_productauf a e           k disk
      * products  (locn,supl,suplsub,prod)

     f*_productcif   e           k disk    rename(rk_product:xk_product)
      * products  (locn,prod)

     f*_suplieraif   e           k disk
      * suppliers (locn,supl,sub)

     f*_locatnsaif   e           k disk
      * locations

     f*k_logprod o  a e             disk
      * log product transactions

     f*_tablcodaif   e           k disk                                         table codes
      * table file

     f*_prodlogauf a e           k disk                                         deal summary info
      * product changes log

     f*_companyauf   e           k disk                                         table codes
      * company file

     f*_transplaif   e           k disk                                         transfer suppliers
      * transfer system regular supplier link

     f*_tranprdaif   e           k disk                                         transfer products
      * transfer system product link

     f*_prodltmauf a e           k disk                                         deal summary info
      * product lead time transactions

     f*_prodforauf a e           k disk                                         forcast change log
      * forcast change log
    ...
    ‚   //open dynamic cursor
      ...
      /copy k3s_c160
       ...
       begsr dclprcursora;
       exec sql
        declare prcursora Cursor
         for DynSQLStmtA;
       endsr;

       begsr PrepDynSQLStmtA;
       exec sql
        Prepare DynSqlStmtA
          From :StmtString;
       endsr;

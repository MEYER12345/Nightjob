     ...
     d prodsez_found   s               n
     d #prep           s              1
     d #cur1unt        s                   like(so_cur1unt) inz
     d #cur1val        s                   like(so_cur1val) inz
     d sddiscbkt       s              1  0
     d sddis1typ       s              1  0
     d sddis1val       s              7  0
     d sddis1unt       s              1  0
     ...
     d #oruptox        s              5  0 inz(0)
     d #intrval        s              5  0 inz(0)
     d #sstimef        s              9  3 inz(0)
     d #cur1typ        s                   like(so_cur1typ) inz(0)
     d #cur2typ        s                   like(so_cur1typ) inz(0)
     d #cursavg        s                   like(so_cursavg) inz(0)
      * -------------------------------------------------------
    ...
    ‚   //open dynamic cursor
       ...
       //PE checks PE1 thru PE5 only (not diverter orders !)
               if (pr_altsrce <> 1) and
                  ((pr_endper = '1') or
                  (pr_endper = '2') or
                  (pr_endper = '3') or
                  ...
                  if SQLState = SQLStateOk;
                     ...
                     if SQLState = SQLStateOk;
                        ...
                        if SQLState = SQLStateOk;
                           if pr_comp = pd_comp and
                              xx_locn = pd_actlocn and
                              xx_supl = pd_actsupl and
                              xx_suplsub = pd_actsub and
                              pr_prod = pd_prod and
                              pd_birth  >= ending_dt  and
                              pd_review  = 0;
       ...
       //  current bracket
             exec sql
               ...
               into   :sddiscbkt, :sddis1typ, :sddis1val, :sddis1unt,
                      ...
                      :sddisrate, :sddissavg
               from k_supldis
               where sd_comp = :sp_comp and
                     sd_locn = :sp_locn and
                     sd_supl = :sp_supl and
                     sd_suplsub = :sp_suplsub and

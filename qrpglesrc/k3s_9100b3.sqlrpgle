       ...
       *inlr = *on;
       ...
       exec sql
          declare alt01cursor cursor
            for
            select substr(k_altsrc01, 9, 11) as ndc_upc01,
                   substr(k_altsrc01, 21, 31) as descrip01,
                   interpret(binary(substr(k_altsrc01, 53, 5)) as numeric(5))
                      as qty_avl01,
                   interpret(binary(substr(k_altsrc01, 61, 3)) as numeric(3))
                      as cost_dl01,
                   interpret(binary(substr(k_altsrc01, 65, 2)) as dec(2,2))
                      as cost_cn01
               from k_altsrc01;
       endsr;

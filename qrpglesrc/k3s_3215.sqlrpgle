     *****************************************************************
      ...
      **   Desc: Read K_PRODFOR J recs, write K recs, update K_PRODUCT
      **         pr_forcast
      *********************************************************************
      *   Maintenance Log.
      ...
      * ---------------------------------------------------------- Begin
       ...
       if bypass_logic = 'N';
          ...
          exec sql
             select cm_sysdate
                ...
                from k_company
                where cm_comp = :comp ---select k_company row based on parm pass
       ...
       // main loop using fetch
        ...
        //fetch k_prodfor row
             ...
             // cursor providing all the records to be processed
                ...
                exec sql
             ---select pr_forcast, pr_fordevp, pr_seasonl, pr_usrstat, pr_syssta
             ---   into :prforcast, :prfordevp, :prseasonl, :prusrstat, :prsysst
       ...
       *inlr = *on;
       ...
       begsr insert_prodhld750;

       exec sql
          insert into prodhld750    --Write product hold out qty record to go to
              (pu_comp,
               pu_locn,
               pu_supl,
               pu_suplsub,
       ...
       begsr insert_prodhld520;

       exec sql
          insert into prodhld520    --Write product hold out qty record to go to
              (pu_comp,
               pu_locn,
               pu_supl,
               pu_suplsub,
       ...
       exsr opnpfcursordiv;     //open cursor
       ...
       //fetch k_prodfor row
          ...
          if SQLState = SQLStateOk and
             ...
             if SQLState = SQLStateOk;
                prod_found = *on;
                if divhold = '750';
                   exsr insert_prodhld750;  //ADC
                   exsr insert_phldlog750;  //ADC
                endif;
                if divhold = '520';
                   exsr insert_prodhld520;  //RDC
                   exsr insert_phldlog520;  //RDC
                endif;
       ...
       endsr;
        ...
        begsr insert_phldlog750;

        exec sql
           insert into phldlog750    --Write product hold out log record to go to ADC
               (h0_comp,
                h0_chgtype,
                h0_birth,
                h0_birthtm,
       ...
       begsr insert_phldlog520;

       exec sql
          insert into phldlog520    --Write product hold out log record to go to RDC
              (h0_comp,
               h0_chgtype,
               h0_birth,
               h0_birthtm,

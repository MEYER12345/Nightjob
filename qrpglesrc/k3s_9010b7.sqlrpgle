     ...
     d  yearend_52                    1
      ...
      /free
       ...
       //                     interface file K_INTPROD
                 exec sql
                    select ta_flag1
                      into :taflag1
                      from k_tablcod
                      where ta_comp = :intprod.ip_comp and
                            ta_codetyp = 'APP' and
       ...
       //  change in location
              if intprod.ip_locn <> locn;
                 locn    = intprod.ip_locn;
       ...
       //  get location
                 exec sql
                   select *
                     into :locatns_rec
                     from k_locatns
                     where lc_comp = :intprod.ip_comp and
                           lc_locn = :intprod.ip_locn
                     fetch first row only;
       ...
       // change in supplier ID
              ...
              #iplocn = intprod.ip_locn;
              #ipsupl = intprod.ip_supl;
              #ipsuplsub = intprod.ip_suplsub;
       ...
       // get supplier record
                 exec sql
                   select *
                     into :suplier_rec
                     from k_suplier
                     where sp_comp = :intprod.ip_comp and
                           sp_locn = :intprod.ip_locn and
                           sp_supl = :intprod.ip_supl and
                           sp_suplsub = :intprod.ip_suplsub
                     fetch first row only;
       ...
       // If location no good, then log error, and by-pass the rest of tests
              pr_deltcnt = 0;
              if bad_locn  = *on;
                 logprod.lg_logtype = '6';
                 logprod.lg_prvsupl = *blanks;
                 logprod.lg_prvsub  = *blanks;
                 logprod.lg_prvsplu = *blanks;
                 logprod.lg_prvsplb = *blanks;
                 logprod.lg_prvname = *blanks;
              // sp_buyr    = 'ZZZ';
                 sp_buyr    = lc_buyrbad;
                 sp_name    = bad_locnd;
                 exsr insert_logprod;
       ...
       //   the new supplier.
                       ...
                       exec SQL
                          select *
                             into :product_rec
                              from k_product
                             where pr_comp = :intprod.ip_comp and
                                   pr_locn = :intprod.ip_locn and
                                   pr_supl = :intprod.ip_supl and
                                   pr_suplsub = :intprod.ip_suplsub and
                                   pr_prod = :intprod.ip_prod
                             fetch first row only;
       ...
       //   look for product record at this location
                          px_comp    = intprod.ip_comp;
                          px_locn    = intprod.ip_locn;
                          px_prod    = intprod.ip_prod;
       ...
       *inlr = *on;
       ...
       pr_suplorg = intprod.ip_supl;
       pr_suplors = intprod.ip_suplsub;

       pr_suplusr = intprod.ip_suplusr;
       pr_suplusb = intprod.ip_suplusb;
       ...
       //  manufactor out flag
       pr_mfgout  = intprod.ip_mfgout;

       //  product sequence
       pr_prodseq = intprod.ip_prodseq;

       //  product description information

       //      product must have first description
       if intprod.ip_desc1 = *blanks;
          logprod.lg_logtype = 'K';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
          ...
          sp_name    = saved_name;
          sp_buyr    = saved_buyr;
          intprod.ip_desc1   = '** Missing description **';
          exsr insert_logprod;
       endif;
       pr_desc1   = intprod.ip_desc1;

       //     only update description 2 if data passed from host system
       if intprod.ip_desc2 <> *blanks;
          pr_desc2 = intprod.ip_desc2;
       endif;

       //     only update manufactor if data passed from host system
       if intprod.ip_mfg <> *blanks;
          pr_mfg = intprod.ip_mfg;
       endif;

       //      default catalog with ndc/upc code
       pr_catalog = intprod.ip_ndc_upc;

       //      only update ndc/upc code if data passed from host system
       if intprod.ip_ndc_upc <> *blanks;
          pr_ndc_upc = intprod.ip_ndc_upc;
       endif;

      //       only update unit of measure if data passed from host system
       if intprod.ip_uom <> *blanks;
          pr_uom = intprod.ip_uom;
       endif;

       //      only update pack size if data passed from host system
       if intprod.ip_packsiz <> *blanks;
          pr_packsiz = intprod.ip_packsiz;
       endif;

       //      only update ti/hi if data passed from host system
       if intprod.ip_tihi <> *blanks;
          pr_tihi = intprod.ip_tihi;
       endif;

       //      only update warehouse location if data passed from host system
       if intprod.ip_whslocn <> *blanks;
          pr_whslocn = intprod.ip_whslocn;
       endif;

       //  product status
       //      product status passed must be an 'R' or 'D'
       //        if not, then an 'R' is assumed
       if intprod.ip_status <> 'R' AND intprod.ip_status <> 'D';
          logprod.lg_logtype = 'V';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       if intprod.ip_status  = 'D' AND pr_sysstat <> 'D';
          pr_sysstat = 'D';
       ...
       //      either 'R', 'L', or 'S'
       if intprod.ip_status  = 'R' AND pr_sysstat = 'D';
      ...
      /copy k3s_c160
       ...
       if intprod.ip_costdiv < 1;
          logprod.lg_logtype = 'L';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       // evalr ip_desc1 = %editc(ip_costdiv:'X');
          exsr insert_logprod;
          intprod.ip_costdiv = 1;
       endif;

       //      don't allow purchase price to be negative
       if intprod.ip_costreg < 0;
          logprod.lg_logtype = 'I';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       if intprod.ip_costreg = 0 and cost_0 = 1;
          logprod.lg_logtype = 'c';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
          ...
          sp_buyr    = saved_buyr;
          if cost_fill  = 1;
             intprod.ip_costreg = cost_value;
             intprod.ip_sales   = cost_value;
          endif;
          exsr insert_logprod;
       endif;

       //      don't allow sales price to be negative
       if intprod.ip_sales   < 0;
          logprod.lg_logtype = 'J';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       if intprod.ip_sales   < intprod.ip_costreg / intprod.ip_costdiv;
          logprod.lg_logtype = 'Y';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       // ip_desc1   = 'Sales price < cost ---------->';
          intprod.ip_sales   = intprod.ip_costreg / intprod.ip_costdiv;
       // move      ip_sales      ip_desc1
          exsr insert_logprod;
       endif;
       ...
       if pr_ovrcreg > 0 and
          ...
          intprod.ip_costreg = pr_ovrcreg;
          intprod.ip_costdiv = pr_ovrcdiv;
       endif;

       if pr_costreg <> intprod.ip_costreg;
       ...
       //    update costs
       pr_costreg = intprod.ip_costreg;
       pr_costdiv = intprod.ip_costdiv;
       pr_costeac = intprod.ip_costreg / intprod.ip_costdiv;

       //    update sales price
       pr_sales   = intprod.ip_sales;

       //    quantity logic

       //    on hand section
       if intprod.ip_qtyohnd < 0;
          logprod.lg_logtype = 'F';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvname = *blanks;
       ...
       if delay_days = 1;
          if pr_sysstat <> 'D' AND
             ...
             if pr_qtyohnd > 0 AND
                intprod.ip_qtyohnd = 0 AND
                pr_birth   < delay_test AND
                intprod.ip_dlysale > 0;

                clear delayuntil;
                callp K3S_3860(intprod.ip_comp:
                               intprod.ip_locn:
                               intprod.ip_supl:
                               intprod.ip_suplsub:
                               sp_buyr:
       ...
       pr_qtyohnd = intprod.ip_qtyohnd;

       //     on order section
       if intprod.ip_qtyoord < 0;
          logprod.lg_logtype = 'G';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       // move      ip_qtyoord    ip_desc1
          exsr insert_logprod;
          clear intprod.ip_qtyoord;
       endif;
       pr_qtyoord = intprod.ip_qtyoord;

       //     back order section
       if intprod.ip_qtyback < 0;
          logprod.lg_logtype = 'H';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
        ...
        //move      ip_qtyback    ip_desc1
          exsr insert_logprod;
          clear intprod.ip_qtyback;
       endif;
       pr_qtyback = intprod.ip_qtyback;

       //  buying multiple
       //      is buying multiple being passed less than 1?
       if intprod.ip_buymult < 1;
          logprod.lg_logtype = 'M';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       // ip_desc1   = 'Invalid buying multiple passed';
          exsr insert_logprod;
          intprod.ip_buymult = 1;
       endif;
       pr_buymuli = intprod.ip_buymult;
       pr_buymult = intprod.ip_buymult;
       if pr_buymulo <> *zeros;
       ...
       if intprod.ip_minqty < 1;
          logprod.lg_logtype = 'N';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       if intprod.ip_purincr < 0;
          logprod.lg_logtype = 'O';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       // ip_desc1   = 'Negative purchase increment';
          exsr insert_logprod;
          intprod.ip_purincr = 0;
       endif;
       pr_purincr = intprod.ip_purincr;
       if pr_purinco <> *zeros;
       ...
       if intprod.ip_disothr < 0;
          logprod.lg_logtype = 'd';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
          ...
          sp_buyr    = saved_buyr;
          exsr insert_logprod;
          intprod.ip_disothr = 0;
       endif;
       pr_disothr = intprod.ip_disothr;
       if pr_disotho <> *zeros;
       ...
       if intprod.ip_disunt7 < 0;
          logprod.lg_logtype = 'e';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
          ...
          sp_buyr    = saved_buyr;
          exsr insert_logprod;
          intprod.ip_disunt7 = 0;
       endif;
       pr_disunt7 = intprod.ip_disunt7;
       if pr_disun7o <> *zeros;
       ...
       if intprod.ip_disunt8 < 0;
          ...
          sp_buyr    = saved_buyr;
          exsr insert_logprod;
          intprod.ip_disunt8 = 0;
       endif;
       pr_disunt8 = intprod.ip_disunt8;
       if pr_disun8o <> *zeros;
       ...
       if intprod.ip_disunt9 < 0;
          ...
          sp_buyr    = saved_buyr;
          exsr insert_logprod;
          intprod.ip_disunt9 = 0;
       endif;
       pr_disunt9 = intprod.ip_disunt9;
       if pr_disun9o <> *zeros;
       ...
       if intprod.ip_rebate  < 0;
       // lg_logtype = 'P';
          logprod.lg_logtype = 'Z';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       pr_rebate  = intprod.ip_rebate;

       //  convenience pack
       //      is convenience pack being passed less than 0?
       if intprod.ip_convpak  < 0;
          logprod.lg_logtype = 'Q';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       // ip_desc1   = 'Neg. convenience pack passed';
          exsr insert_logprod;
          intprod.ip_convpak = 0;
       endif;
       pr_convpak = intprod.ip_convpak;
       if pr_convpko <> *zeros;
       ...
       if intprod.ip_weightd  < 1;
          logprod.lg_logtype = 'S';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       // ip_desc1   = 'Weight divisor less than 1';
          exsr insert_logprod;
          intprod.ip_weightd = 1;
       endif;

       //      is weight being passed less than 0?
       if intprod.ip_weight  < 0;
          logprod.lg_logtype = 'R';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       pr_weight  = intprod.ip_weight;
       pr_weightd = intprod.ip_weightd;
       ...
       if intprod.ip_volumed  < 1;
          logprod.lg_logtype = 'U';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       // ip_desc1   = 'Volume divisor less than 1';
          exsr insert_logprod;
          intprod.ip_volumed = 1;
       endif;

       //    is volume being passed less than 0?
       if intprod.ip_volume  < 0;
          logprod.lg_logtype = 'T';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       pr_volume  = intprod.ip_volume;
       pr_volumed = intprod.ip_volumed;
       ...
       // move      ip_dlysale    ip_desc1
          exsr insert_logprod;
          clear intprod.ip_dlysale;
       endif;

       //      is data valid (negative outs being passed?)
       if intprod.ip_dlyouts < 0;
          logprod.lg_logtype = 'E';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
       ...
       //      add daily sales
       pr_accsale += intprod.ip_dlysale;
       //      add daily lost sales
       pr_accouts += intprod.ip_dlyouts;
       //      demand for period
       ...
       //  days out logic
                if (intprod.ip_qtyohnd <= pr_daysunt) and
                   (pr_sysstat <> 'N') and
                   (pr_forcast > *zeros) and
                   continue = 1;
       ...
       //  contract existance
       pr_contflg = intprod.ip_contflg;

       //  car count flag (must be 0=no or 1=yes)
       if car_update = 1;
          pr_carcoun = intprod.ip_carcoun;
          if pr_carcoun <> 0 and
             pr_carcoun <> 1;
             pr_carcoun =  0;
          endif;
       ...
       //    ip_desc1   = 'Invalid flag alt srce process';
             intprod.ip_procalt = 0;
             exsr insert_logprod;
          endif;
          pr_procalt = intprod.ip_procalt;
       endif;
       ...
       //     go check for another deal
          ...
          callp K3S_5120(intprod.ip_comp:
                         intprod.ip_locn:
                         intprod.ip_supl:
                         intprod.ip_suplsub:
                         intprod.ip_prod:
       ...
       logprod.lg_logtype = '2';
       logprod.lg_prvsupl = sp_supl;
       logprod.lg_prvsub  = sp_suplsub;
       logprod.lg_prvsplu = sp_supl;
       logprod.lg_prvsplb = sp_suplsub;
       logprod.lg_prvname = sp_name;
       sp_name    = saved_name;
       sp_buyr    = saved_buyr;
       exsr insert_logprod;
       ...
       //     monthly and 13-four weekly products
          callp K3S_3510(intprod.ip_comp:
                         intprod.ip_locn:
                         px_supl:
                         px_suplsub:
                         intprod.ip_prod:
       ...
       //     weekly products
          callp K3S_3511(intprod.ip_comp:
                         intprod.ip_locn:
                         px_supl:
                         px_suplsub:
                         intprod.ip_prod:
       ...
       pr_supl    = intprod.ip_supl;
       pr_suplsub = intprod.ip_suplsub;
       pr_suplusr = intprod.ip_suplusr;
       pr_suplusb = intprod.ip_suplusb;
       pr_suplorg = intprod.ip_supl;
       pr_suplors = intprod.ip_suplsub;
       ...
       // key information
       pr_comp    = intprod.ip_comp;
       pr_locn    = intprod.ip_locn;
       pr_supl    = intprod.ip_supl;
       pr_suplsub = intprod.ip_suplsub;
       pr_suplusr = intprod.ip_suplusr;
       pr_suplusb = intprod.ip_suplusb;
       ...
       //         should be:  1 = weekly
       //                     2 = monthly
       if intprod.ip_forctyp < 1 OR intprod.ip_forctyp > 3;
          logprod.lg_logtype = 'W';
          logprod.lg_prvsupl = *blanks;
          logprod.lg_prvsub  = *blanks;
          logprod.lg_prvsplu = *blanks;
          logprod.lg_prvsplb = *blanks;
       ...
       //       weekly forecasting
          when intprod.ip_forctyp = 1;
               pr_forcint = 52;
               pr_forcper = lc_perd52;
               pr_forcyr  = lc_year52;

       //       monthly forecasting
          when intprod.ip_forctyp = 2;
               pr_forcint = 12;
               pr_forcper = lc_perd12;
               pr_forcyr  = lc_year12;

       //       thirteen four-weekly forecasting
          when intprod.ip_forctyp = 3;
               pr_forcint = 13;
      ...
      /copy k3s_c160
       ...
       select;
          ...
          when logprod.lg_logtype = '1';
               lda_rctyp1 += 1;

          when logprod.lg_logtype = '2';
               lda_rctyp2 += 1;

          when logprod.lg_logtype = '5';
               lda_rctyp5 += 1;

          when logprod.lg_logtype = '6';
               lda_rctyp6 += 1;

          when logprod.lg_logtype = '7';
               lda_rctyp7 += 1;

          when logprod.lg_logtype = '8';
               lda_rctyp8 += 1;

          when logprod.lg_logtype = '9';
               lda_rctyp9 += 1;
       ...
       logprod.lg_comp    = intprod.ip_comp;
       logprod.lg_locn    = intprod.ip_locn;
       logprod.lg_buyr    = sp_buyr;
       logprod.lg_supl    = intprod.ip_supl;
       logprod.lg_suplsub = intprod.ip_suplsub;
       logprod.lg_suplusr = intprod.ip_suplusr;
       logprod.lg_suplusb = intprod.ip_suplusb;
       logprod.lg_suplnam = sp_name;
       logprod.lg_prod    = intprod.ip_prod;
       logprod.lg_desc1   = intprod.ip_desc1;
       logprod.lg_deltcnt = pr_deltcnt;

       // call module to retrieve timestamp
       callp K3S_Retrieve_Timestamp(time_stamp);
       logprod.lg_timestp = time_stamp;
       ...
       if get_log_5 = '1' AND lg_logtype = '5';
          callp K3S_9010X1(intprod.ip_comp:
                           intprod.ip_locn:
                           intprod.ip_suplusr:
                           intprod.ip_suplusb:
                           intprod.ip_prod:
        ...
        if idx = lgArySize;
           ...
           if ndx = RowsFetched and RowsFetched < ArraySize;
              if idx < lgArySize;
                 exec sql
                    insert into k_logprod
                      :lgArySize rows
